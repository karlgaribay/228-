...
```{r po, echo=TRUE}
eff_size <- 1.5 #We believe the treated fisheries will have an FPI score that is higher by 1.5 than the control fisheries. Thus, the ATE = 1.5.
potential_outcomes <- 
  declare_potential_outcomes(
    Y_D_0=fpi + u,
...
hist(sim.out$estimate[sim.out$estimator=="DIM"],
     ...
     xlim=c(1,2), cex=24)
abline(v=1.5, lwd=3, col="red") #Why are both of the distributions for DIM and DIM the same?
```
...
```{r other-mde1}
diagnoses2 <- design %>% 
  redesign(eff_size=seq(0.1, 1.5, 0.1)) %>% 
