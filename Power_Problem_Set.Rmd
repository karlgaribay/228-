...
population <- declare_population(
  waterways = add_level(N=500, #The population consists of 500 villages.
    tree_cover_ha=rtruncnorm(n=N, a=40, b=400, 
                            mean=140, sd=145), #Each village has some baseline level of twater quality. We're assuming that water quality across all streams follows a truncated normal distribution -- one that does not exceed 40 and 400 at its tails, and has a mean of 140 ha and a standard deviation of 145 ha.
    u=rnorm(n=N, mean=-13.4, sd=13.4)) #Village-level variability (for calculating the treatment effect -- see below).
)
```
...
estimand <- declare_inquiry(ATE = eff_size) #PH: We're declaring our estimand here, which is equal to the effect size (5.5, or the treatment effect from the Jayachandran et al. sutdy.)
estimand(po)
```
...
did <- declare_estimator(Y - tree_cover_ha ~ D, #PH: Differences in differences estimator because this equation estimates the difference in the difference between endline and basline tree cover between treatement and control villages.
                         inquiry = estimand,
                         model = difference_in_means, 
                         label = "DID")
```
...
```{r more-sample}
samp_size <- 500 #PH: Redefining our sample size to 500 villages.
sampling2 <- 
...
diagnoses <- design %>% 
  redesign(samp_size=seq(100,1000,100)) %>% 
